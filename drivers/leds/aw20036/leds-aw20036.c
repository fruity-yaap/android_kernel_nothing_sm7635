...
#define AW20036_BLACK_IMAX AW20036_IMAX_80mA
#define AW20036_WHITE_IMAX AW20036_IMAX_80mA
...
static int aw20036_led_init(struct aw20036 *aw20036)
{
	...
	if(strncmp(dev_color, "BLACK", 5) == 0){
		...
		aw20036_imax_cfg(aw20036, AW20036_BLACK_IMAX);
	}else{
		aw20036_imax_cfg(aw20036, AW20036_WHITE_IMAX);
	}
	...
	pr_info("%s handset_mode:%d\n", __func__, aw20036->handset_mode);
	if (aw20036->handset_mode == 1) {
		/*When handset mode, use 555Hz*/
		aw20036_i2c_write(aw20036, 0x81, 0x57);
		aw20036_i2c_write(aw20036, 0x82, 0x3B);
		aw20036_i2c_write(aw20036, 0x83, 0xFF);
		aw20036_i2c_write(aw20036, 0x84, 0x04);
		aw20036_i2c_write(aw20036, 0x85, 0x28);
		aw20036_i2c_write(aw20036, 0x86, 0x03);
	} else {
		/*When handset mode, use 9KHz*/
		aw20036_i2c_write(aw20036, 0x81, 0x7F);
		aw20036_i2c_write(aw20036, 0x82, 0x3B);
		aw20036_i2c_write(aw20036, 0x83, 0xDE);
		aw20036_i2c_write(aw20036, 0x84, 0x00);
		aw20036_i2c_write(aw20036, 0x85, 0xBE);
		aw20036_i2c_write(aw20036, 0x86, 0x00);
	}
					      ...
					      const char *buf, size_t len)
{
	...
	struct led_classdev *led_cdev = dev_get_drvdata(dev);
	struct aw20036 *aw20036 = container_of(led_cdev, struct aw20036, cdev);
	unsigned char data[ALL_CHANNEL];

	sscanf(buf, "%d", &val);
	pr_info("%s: %d\n", __func__, val);

	for (i = 0; i < ALL_CHANNEL; i ++) {data[i] = val;}

	aw20036_reg_page_cfg(aw20036, AW20036_REG_PAGE2);
	aw20036_i2c_write_block(aw20036, 0x00, ALL_CHANNEL, data); /*led(0-36)*/
					      ...
					      const char *buf, size_t len)
{
	...
	aw20036_reg_page_cfg(aw20036, AW20036_REG_PAGE2);
	aw20036_i2c_write_block(aw20036, 0x00, ALL_CHANNEL, data);
	return len;
}
					      ...
					      const char *buf, size_t len)
{
	...
	aw20036_reg_page_cfg(aw20036, AW20036_REG_PAGE2);
	aw20036_i2c_write_block(aw20036, 0x00, ALL_CHANNEL, brightness); /*led(0-35)*/
					      ...
					      const char *buf, size_t len)
{
	...
	if(strncmp(dev_color, "BLACK", 5) == 0){
		...
		aw20036_imax_cfg(aw20036, AW20036_WHITE_IMAX);//default is white
	}
	aw20036_rgbcolor_config(aw20036);
...
static void aw20036_lamps_flowing_work(struct work_struct *work)
{
	struct aw20036 *aw20036 = container_of(work, struct aw20036,
			lamps_flowing_work);
	int ret = 0;
	int led_off = 0;
	int brightness = factory_led5;
	unsigned int num;
	int led[ALL_CHANNEL] = {18, 6, 29, 17, 5, 28, 16, 4, 27, 15, 3, 26, 14, 2, 25, 13, 1, 24, 12, 0, 8, 9, 21, 33, 32, 34, 10, 22, 11, 23, 35, 20, 31, 30, 19, 7};

	pm_stay_awake(aw20036->dev);
	aw20036->flowing_work_state = true;
	...
	for(num=0; num<20; num++){
		ret = aw20036_i2c_write(aw20036, led[num], brightness);
		if(ret < 0){
			goto end;
		}
		if(!((num + 1) % 3))
	...
	//led0
	for(num=0; num<20; num++){
		ret = aw20036_i2c_write(aw20036, led[num], led_off);
		if(ret < 0){
			goto end;
		}
		if(!((num + 1) % 3))
	...
	aw20036->flowing_work_state = false;
	pm_relax(aw20036->dev);
}
					      ...
					      const char *buf, size_t len)
{
	...
	if (sscanf(buf, "%d %d %d %d %d",
		...
		if(!aw20036->flowing_work_state) {
			for(num=0; num<5; num++){
				ret = aw20036_i2c_write(aw20036, led1[num], f_cam_led_br);
				if(ret < 0){
					return ret;
				}
			}
		...
		if (red_led_br && !r_cam_leds_br && !aw20036->flowing_work_state) {
			schedule_work(&aw20036->lamps_flowing_work);
		}
...
static ssize_t aw20036_handset_mode_show(struct device *dev,
				 struct device_attribute *attr, char *buf)
{
	struct led_classdev *led_cdev = dev_get_drvdata(dev);
	struct aw20036 *aw20036 = container_of(led_cdev, struct aw20036, cdev);

	pr_info("%s aw20036->handset_mode:%d\n", __func__, aw20036->handset_mode);

	return sprintf(buf, "%d\n", aw20036->handset_mode);
}

static ssize_t aw20036_handset_mode_store(struct device *dev,
				  struct device_attribute *attr,
				  const char *buf, size_t len)
{
	struct led_classdev *led_cdev = dev_get_drvdata(dev);
	...
	pr_info("%s handset_mode:%d -> %d (op_mode %d)\n", __func__, aw20036->handset_mode, val, aw20036->operating_mode);

	if (val && (!aw20036->handset_mode)) {
		/*When handset mode, use 555Hz*/
		aw20036->handset_mode = 1;
		if (aw20036->operating_mode != 0) {
			aw20036_reg_page_cfg(aw20036, AW20036_REG_PAGE0);
			aw20036_i2c_write(aw20036, 0x81, 0x57);
			aw20036_i2c_write(aw20036, 0x82, 0x3B);
	...
	} else if (!val && (aw20036->handset_mode)) {
		 /*When handset mode, use 9KHz*/
		aw20036->handset_mode = 0;
		if (aw20036->operating_mode != 0) {
			aw20036_reg_page_cfg(aw20036, AW20036_REG_PAGE0);
			aw20036_i2c_write(aw20036, 0x81, 0x7F);
			aw20036_i2c_write(aw20036, 0x82, 0x3B);
...
static DEVICE_ATTR(handset_mode, 0664, aw20036_handset_mode_show, aw20036_handset_mode_store);
				  ...
				  struct device_node *np)
{
	...
	aw20036->operating_mode =1;
#endif
	INIT_WORK(&aw20036->lamps_flowing_work, aw20036_lamps_flowing_work);
...
static void aw20036_leds_effect_work(struct work_struct *work)
{
	...
	ktime_t start, runtime, delay;
       int i=0, j=0, num=0;
	int led_36[ALL_CHANNEL] =  {18, 6, 29, 17, 5, 28, 16, 4, 27, 15, 3, 26, 14, 2, 25, 13, 1, 24, 12, 0, 8, 9, 21, 33, 32, 34, 10, 22, 11, 23, 35, 20, 31, 30, 19, 7};
	...
	if(aw20036->start_buf->status == MMAP_BUF_DATA_FINISHED) {
		flag = 1;
		aw20036->start_buf->status = MMAP_BUF_DATA_VALID;
		pr_info("%s: Ignore the MMAP_BUF_DATA_FINISHED judgment once!", __func__);
	}
	do {
		if(aw20036->curr_buf->status == MMAP_BUF_DATA_VALID) {
			if(flag == 1) {
				aw20036->start_buf->status = MMAP_BUF_DATA_FINISHED;
				flag = 0;
				pr_info("%s: Restore start_buf to MMAP_BUF_DATA_FINISHED", __func__);
			}
			pr_info("%s aw20036->curr_buf->length:%d\n", __func__,  aw20036->curr_buf->length);
			if(aw20036->sec_num == ALL_CHANNEL){
				for (i = 0; i < aw20036->curr_buf->length; i += ALL_CHANNEL){
					start = ktime_get();
					for(j=0; j < ALL_CHANNEL; j++){
					       num = led_36[j];
					       brightness[num] = aw20036->curr_buf->data[i+j]*aw20036->curr_buf->brightness/MAX_BRIGHTNESS;
					}
				...
				if((aw20036->curr_buf->length < 468) ||(aw20036->stream_mode ==0)){
					pr_info("%s break 3\n", __func__);
					break;
				}
			}
